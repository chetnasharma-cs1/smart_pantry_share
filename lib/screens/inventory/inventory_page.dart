      ...
      for (TextBlock block in recognizedText.blocks) {
        for (TextLine line in block.lines) {
          if (line.text.contains('ITEMS -')) {
            ...
            products = itemsText.split(',').map((item) => item.trim()).where((item) => item.isNotEmpty).toList();
...
class _ProductSelectionDialog extends StatefulWidget {
  ...
  const _ProductSelectionDialog({Key? key, required this.products}) : super(key: key);
...
class AddItemDialog extends StatefulWidget {
  ...
  const AddItemDialog({super.key, required this.onAdd, this.initialItem, this.initialItemName});
